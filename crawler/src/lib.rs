...
pub async fn start_craw(data_dir: &str) {
    ...
    let tasks = SEASON_LIST.into_iter().map(|season_url| {
        ...
        async move {
            ...
            // tasks for get game info
            let tasks = game_id_list.into_iter().map(|game_id| {
                let form = form.clone();
                let regex = regex.clone();
                let client = client.clone();
                async move {
                    ...
                    if !path.exists() {
                        ...
                        info!("{} finished in {} ms.", game_id, dur);
                    }
                }
            });
            ...
            let dur = instant.elapsed().as_millis();
            info!("{} finished in {} ms.", season_url, dur);
        }
    });
    ...
    let dur = instant.elapsed().as_secs();
